...
class SonarSweep
{
  ...
  public function getAnswer1(bool $answer2=false):string
  {
    ...
    foreach ($aLines as $line) {
      if (!is_null($prev) && intval($line) > intval($prev)) {
        $inc++;
      }
      $prev = $line;
    }
  ...
  public function getAnswer2(bool $answer2=false):string
  {
    $aLines = explode("\n", $this->input);
    $c = 0;
    for ($inc=3; $inc<count($aLines);$inc++){
      $c += array_sum([$aLines[$inc],$aLines[$inc-1],$aLines[$inc-2]]) > array_sum([$aLines[$inc-1],$aLines[$inc-2],$aLines[$inc-3]]);
    }
    return ''.$c;
  }
}